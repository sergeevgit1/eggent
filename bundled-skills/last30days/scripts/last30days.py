...
def _search_web(
    ...
    Uses the best available backend: Parallel AI > Brave > Tavily > OpenRouter.
...
def main():
    ...
    if not web_source:
        source_info["web_skip_reason"] = (
            "assistant will use WebSearch "
            "(add BRAVE_API_KEY / OPENROUTER_API_KEY / PARALLEL_API_KEY / TAVILY_API_KEY for native search)"
        )
